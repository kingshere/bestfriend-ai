...
function SidePanel({ 
  ...
  console.log("SidePanel props:", { 
    isOpen, 
    previousConversationsCount: previousConversations.length, 
    activeConversationId,
    isLoading 
  });
  
  const handleConversationClick = (id) => {
    console.log(`Clicking conversation with ID: ${id}`);
    loadConversation(id);
  };
  ...
  return (
    <div className={`fixed md:static top-20 bottom-0 left-0 z-20 w-64 bg-teal-900 shadow-xl transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} overflow-y-auto`}>
      ...
      <div className="p-2">
        ...
        {isLoading ? (
          ...
          previousConversations.map(conv => (
            ...
            >
              <div className="pr-8 truncate">{conv.title || "Untitled Conversation"}</div>
              <button 
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2 text-xs text-center text-teal-400 border-t border-teal-700">
        {isLoading ? "Loading..." : previousConversations.length > 0 ? `${previousConversations.length} conversations loaded` : "No conversations found"}
      </div>
    </div>
  );
}
